...
import { AISearchAPIClient, ResponseType } from "aisearchapi-client";
...
class AISearchMCPServer {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      const tools: Tool[] = [
          ...
          inputSchema: {
            ...
            properties: {
              prompt: {
                type: "string",
                description:
                  "The main search query that drives the embedding process. This content is parsed and transformed into vector representations for similarity matching and retrieval.",
              },
              use_context: {
                type: "boolean",
                description:
                  "Whether to include conversation history as context for better understanding. When true, uses the last few messages to provide contextual awareness.",
                default: false,
              },
              additional_context: {
                type: "array",
                ...
                description:
                  "Optional additional conversation context to enhance the model's understanding. An ordered list of messages that influence the final response.",
              },
              response_type: {
                type: "string",
                enum: ["text", "markdown"],
                description:
                  "Response format preference. 'markdown' returns rich formatted text with styling (default), 'text' returns plain text without formatting.",
                default: "markdown",
              },
              include_sources: {
                type: "boolean",
              ...
              include_timing: {
                type: "boolean",
                description:
                  "Whether to include processing time information in the response",
                default: false,
              },
            },
            required: ["prompt"],
        ...
        },
        {
          ...
          description:
            "Check your current AI Search API account balance and available credits. Useful for monitoring API usage and ensuring sufficient credits are available for operations.",
          inputSchema: {
            type: "object",
            properties: {
              format: {
                ...
                description:
                  "Format of the balance response. 'detailed' includes usage recommendations, 'simple' just shows the number.",
                default: "detailed",
              },
            },
          },
        },
        {
          ...
          description:
            "Clear the conversation context history. Useful when starting a new topic or when context is no longer relevant.",
          inputSchema: {
            type: "object",
            properties: {},
          },
  ...
  private async handleSearch(request: CallToolRequest): Promise<any> {
    const args = request.params.arguments ?? {};
    ...
    if (progressToken) {
      this.server.notification({
        ...
        params: {
          progressToken,
          value: { message: "Starting search request", percent: 0 },
        },
      });
    }
    ...
    if (progressToken) {
      this.server.notification({
        ...
        params: {
          progressToken,
          value: { message: "Sending query to API", percent: 30 },
        },
      });
    }
    ...
    const response = await this.apiClient.search({
      ...
      response_type:
        (args.response_type?.toString() as ResponseType) || "markdown",
    });
    ...
    if (progressToken) {
      this.server.notification({
        ...
        params: {
          progressToken,
          value: { message: "Processing API response", percent: 70 },
        },
      });
    }

    // Store query in context
    this.addToContext(args.prompt?.toString() ?? '');
    ...
    if (progressToken) {
      this.server.notification({
        ...
        params: {
          progressToken,
          value: { message: "Finalizing", percent: 90 },
        },
      });
    }
  ...
  private formatBalanceResponse(response: AISearchBalanceResponse): string {
    ...
    output.push(
      `• Complex searches with context: ~${Math.floor(
        response.available_credits * 0.5
      )} queries`
    );
  ...
  const tools = [
    {
      name: "aisearch-search",
      description:
        "Performs AI-powered searches with semantic understanding and context awareness. Features include markdown/text formatting, conversation context integration, source citations, and processing time metrics. Perfect for research, documentation lookup, and complex question answering.",
    },
    {
      name: "aisearch-balance",
      description:
        "Checks your AI Search API account balance and available credits. Provides detailed or simple format options with usage recommendations and estimated remaining queries based on current credit level.",
    },
    {
      name: "aisearch-clear-context",
      description:
        "Clears the conversation context history. Useful when switching topics or when previous context is no longer relevant for new searches.",
    },
  ];
